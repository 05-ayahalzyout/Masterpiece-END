...
namespace MVC_MAS_AYAH.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            string x = form["SupAdmin"];
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PhoneNumber = model.Phone };
                var result = await UserManager.CreateAsync(user, model.Password);
                string userID = user.Id;
                int Rols = Convert.ToInt32(x);
                if (result.Succeeded)
                {
                    if (x =="2")
                    {
                        AspNetUserRole userRols = new AspNetUserRole();
                        userRols.UserId = user.Id;
                        userRols.RoleId = x;
                        db.AspNetUserRoles.Add(userRols);
                        Service_owners service_Owners = new Service_owners();
                        service_Owners.Asp_Id = user.Id;
                        service_Owners.Service_Id = db.Servicesses.Select(a=>a.Id).Min();
