...
namespace MVC_MAS_AYAH.Controllers
{
    public class Service_owners1Controller : Controller
    {
        ...
        {
            return View("Index", db.Service_owners.Where(p => p.Photograph.Contains(search) || p.AspNetUser.PhoneNumber.Contains(search) || p.Servicess.Service_name.Contains(search) || p.Number.Contains(search) || p.Descripion.Contains(search) || p.AspNetUser.Email.Contains(search) || p.Image_to_work.Contains(search) || p.Image_to_work10.Contains(search) || p.Image_to_work2.Contains(search) || p.Image_to_work3.Contains(search)).ToList());
        }
